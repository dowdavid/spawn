...
import {
  ...
  accent, textMuted, textTertiary, textDimmed, textBody,
  inputBg, inputBorder, inputText,
  fontMono, fontSizeBase, fontSizeCode, fontSizeSmall, fontSizeXSmall,
  titleBarPadding, titleBarGap, iconSize, closeBtnPadding, smallRadius, buttonRadius,
} from './theme';
...
const ACCENT_COLOR = accent.browser;
...
export async function createBrowserNode(
  ...
  typeIcon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="${iconSize}" height="${iconSize}" viewBox="0 0 24 24" fill="none" stroke="${textMuted}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/></svg>`;
  ...
  urlInput.style.cssText = `flex:1;background:${inputBg};border:1px solid ${inputBorder};border-radius:${smallRadius}px;color:${inputText};font-family:${fontMono};font-size:${fontSizeBase}px;padding:4px 10px;outline:none;min-width:0;`;
  ...
  refreshBtn.style.cssText = `display:flex;align-items:center;cursor:pointer;padding:${closeBtnPadding}px;border-radius:${smallRadius}px;flex-shrink:0;`;
  refreshBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="${iconSize}" height="${iconSize}" viewBox="0 0 24 24" fill="none" stroke="${textMuted}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/></svg>`;
  refreshBtn.addEventListener('mouseenter', () => { refreshBtn.querySelector('svg')!.style.stroke = ACCENT_COLOR; });
  refreshBtn.addEventListener('mouseleave', () => { refreshBtn.querySelector('svg')!.style.stroke = textMuted; });
  ...
  closeBtn.style.cssText = `display:flex;align-items:center;cursor:pointer;padding:${closeBtnPadding}px;border-radius:${smallRadius}px;flex-shrink:0;`;
  ...
  statusText.style.cssText = `color:${textMuted};font-family:${fontMono};font-size:${fontSizeCode}px;text-align:center;padding:24px;`;
...
function loadUrl(id: string, url: string): void {
  ...
  if (isLocalUrl(url)) {
    ...
    iframe.style.cssText = `
      ...
      border-radius:0 0 ${cornerRadius - borderWidth}px ${cornerRadius - borderWidth}px;
    `;
    ...
    icon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="${textMuted}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" x2="21" y1="14" y2="3"/></svg>`;
    fallback.appendChild(icon);

    const msg = document.createElement('div');
    msg.style.cssText = `color:${textTertiary};font-family:${fontMono};font-size:${fontSizeSmall}px;text-align:center;max-width:300px;line-height:1.5;`;
    ...
    openBtn.style.cssText = `
      background:${ACCENT_COLOR};color:#000;border:none;border-radius:${buttonRadius}px;
      padding:10px 22px;font-family:${fontMono};font-size:${fontSizeBase}px;
    ...
    urlHint.style.cssText = `color:${textDimmed};font-family:${fontMono};font-size:${fontSizeXSmall}px;word-break:break-all;text-align:center;max-width:300px;`;
...
function commitUrl(id: string, newUrl: string): void {
  ...
  if (newUrl) {
    ...
    data.statusText.style.color = textMuted;
  }
}
